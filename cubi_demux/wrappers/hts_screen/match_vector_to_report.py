...
KEYS = ['unmapped', 'one_one', 'one_multi', 'multi_one',
        'multi_multi']
...
def run(args):
    ...
    # print results
    print('#hts_screen version 0.1\t#Reads in subset: {num_reads}'.format(
            num_reads=num_reads), file=args.out_file)
    print(('Genome\t#Reads_processed\t#Unmapped\t%Unmapped\t'
    ...
    for i, genome in enumerate(genomes):
        ...
        print(tpl.format(
            genome, num_reads,
            counts[i]['unmapped'], 100 * counts[i]['unmapped'] / num_reads,
            counts[i]['one_one'], 100 * counts[i]['one_one'] / num_reads,
            counts[i]['multi_one'], 100 * counts[i]['multi_one'] / num_reads,
            counts[i]['one_multi'], 100 * counts[i]['one_multi'] / num_reads,
